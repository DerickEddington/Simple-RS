...
impl Parser {
    ...
    // return expr ;
    fn parse_return(&mut self) -> usize {
    ...
    // Create ReturnNode
    fn new_return(&mut self, ctrl: usize, data: usize) -> usize {
        let nid = self._nodes.len();
        let node = Box::new(Node::ReturnNode {
            _data: NodeData {
                _nid: nid,
    ...
    // Create ConstantNode
    fn new_constant(&mut self, value: i64) -> usize {
        let nid = self._nodes.len();
        let node = Box::new(Node::ConstantNode {
            _data: NodeData {
                _nid: nid,
    ...
    // create StartNode
    fn new_start(&mut self) -> usize {
        let nid = self._nodes.len();
        let node = Box::new(Node::StartNode {
            _data: NodeData {
                _nid: nid,
                _inputs: vec![],
                _outputs: vec![],
